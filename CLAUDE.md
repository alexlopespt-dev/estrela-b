...
- **Lesões → Sheets** (secção 12 do .gs, test22): cada gravação da app que mexe em `injuries`/`players`/`meta` copia as lesões para o separador "· Lesões" do Painel (linhas com `app:id` na coluna F; linhas à mão intactas; nome pela mesma regra de ligação da monitorização; ativa→Lesionado, condicionado→Condicionado, alta→data de fim) e agenda `atualizarDaApp` ~1 min depois. Menu: "Copiar lesões da app para o separador Lesões".
