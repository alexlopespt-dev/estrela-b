...
function exForm(id){
  ...
  const body=`<div class="form">
    ...
    <label class="fld full">Objetivo<textarea name="obj" style="min-height:48px">${esc(x.obj||"")}</textarea></label>
    <label class="fld full">Descrição / organização<textarea name="desc">${esc(x.desc||"")}</textarea></label>
    <label class="fld full">Pontos-chave e variantes<textarea name="cp">${esc(x.cp||"")}</textarea></label>
    <div class="fld full">Momento(s) trabalhado(s)<div class="momchk">${MOMENTS.map(m=>`<label title="${esc(m.l)}"><input type="checkbox" name="mom_${m.k}" ${exMoms(x).includes(m.k)?"checked":""}><i class="mdot" style="background:var(--m-${m.k})"></i>${m.ab} <span class="small muted" style="font-weight:500">${esc(m.l)}</span></label>`).join("")}</div></div></div>
    ${prChecks(x.pr||[])}`;
