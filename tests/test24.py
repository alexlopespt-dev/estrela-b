...
with sync_playwright() as pw:
    ...
    # escolher o momento diretamente no treino (sem ir aos exercícios)
    pg.set_viewport_size({"width":1280,"height":950})
    pg.evaluate(f"""(()=>{{ const d=JSON.parse(localStorage.getItem('{LSK}')); d.events['{tid}'].plan=[{{ex:'exi002',name:'Conquistar a base',min:20}},{{ex:null,name:'Bloco livre',min:10}}]; localStorage.setItem('{LSK}',JSON.stringify(d)); }})()""")
    pg.reload(); pg.wait_for_timeout(900); pg.click('nav [data-t="treinos"]'); pg.click(f'[data-p="treino"][data-id="{tid}"]'); pg.wait_for_timeout(400)
    pg.click(f'[data-a="planMom"][data-i="0"][data-k="trd"]'); pg.wait_for_timeout(250)
    pg.click(f'[data-a="planMom"][data-i="1"][data-k="fbp"]'); pg.wait_for_timeout(250)
    pg.click(f'[data-a="planMom"][data-i="1"][data-k="oo"]'); pg.wait_for_timeout(250)
    pg.click(f'[data-a="planMom"][data-i="1"][data-k="oo"]'); pg.wait_for_timeout(250)   # desmarcar
    d=LS(); pl=d["events"][tid]["plan"]; print("no treino:", [x.get("mom") for x in pl], "| exercício ficou com:", d["exercises"]["exi002"].get("mom"))
    if pl[0].get("mom")!=["trd"] or pl[1].get("mom")!=["fbp"] or d["exercises"]["exi002"].get("mom")!=["trd"]: errs.append("momento no treino")
    print("botões ativos:", pg.eval_on_selector_all('[data-a="planMom"].on',"e=>e.map(b=>b.dataset.i+b.dataset.k)"))
    pg.screenshot(path=os.path.join(ROOT,"tests","capturas","t24_treino.png"))
    pg.click('nav [data-t="treinos"]'); pg.click('[data-a="tsub"][data-k="plan"]'); pg.wait_for_timeout(400)
    leg=" ".join(pg.inner_text(".mleg >> nth=-1").split("\n")); print("gráfico:", leg)
    if "66.7%" not in leg or "33.3%" not in leg: errs.append("gráfico com momentos do treino")
