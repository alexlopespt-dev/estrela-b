...
with sync_playwright() as pw:
    ...
    nsel=pg.eval_on_selector_all("#dlg [name=bpsel]:checked","e=>e.length"); print("escolher para o PDF:", nsel, "marcadas |", pg.inner_text("#bpPrN"))
    if nsel!=len(SP(pg)): errs.append("pré-seleção")
    pg.screenshot(path=os.path.join(CAP,"t28_pdf_escolher.png"))
    ...
    if html.count("<svg")<9 or 'class="bpidx"' not in html: errs.append("pdf todas")
    # só algumas: nenhuma -> escolher 2 (penálti + o livre de 3 passos), seguidas
    pen=[i for i,x in SP(pg).items() if x["type"]=="pen"][0]
    pg.click('[data-a="bpPrintAll"]'); pg.wait_for_timeout(300); pg.click('#dlg [data-a="bpPrSel"][data-k=""]')
    pg.click('#dlg [data-a="bpPrGo"][data-k="dl"]'); pg.wait_for_timeout(200)
    if "pelo menos" not in pg.inner_text("#toast") or not pg.evaluate("document.querySelector('#dlg').open"): errs.append("nenhuma escolhida")
    pg.check(f'#dlg [name=bpsel][value="{pen}"]'); pg.check(f'#dlg [name=bpsel][value="{lid}"]'); pg.click('#dlg [data-a="bpPrLay"][data-k="2"]')
    print("contador:", pg.inner_text("#bpPrN"))
    if "2 bolas paradas" not in pg.inner_text("#bpPrN"): errs.append("contador")
    with pg.expect_download() as dl: pg.click('#dlg [data-a="bpPrGo"][data-k="dl"]')
    html=open(dl.value.path()).read(); names=re.findall(r'<li><b>(.*?)</b>',html)
    print("PDF escolhidas:", html.count("<svg"), "campos |", names, "| seguidas:", "page-break-after:always}" not in html)
